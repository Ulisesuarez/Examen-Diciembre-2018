...
const universoClass = require('./universo');
const singleton = require('./singletonFactory');
const utilities = require('./objectUtilities');
/**
 * Crea el objeto Rick
 */
 ...
 */
const clonRick = characters.Clone(ricksSingleton);
...
const otroRick = characters.Clone(ricksSingleton);
...
const clonMorty = characters.Clone(MortySingleton);
 ...
 */
const universoSingleton = singleton.singletonFactory(universoClass.Universo);
const universo = universoSingleton.get();
 ...
 */
const tierra = [protoRick, clonRick, otroRick, protoMorty, clonMorty, jerry];
...
console.assert(universo.hasOwnProperty("Tierra"));
console.log(universo);
...
gun.setDimensionHistorial('Tierra');
console.assert(gun);
console.assert(gun.historial.length === 1);
protoRick.disparar(gun, "Tierra", "Fart", ['Jerry']);
console.assert(universo.hasOwnProperty("Fart"));
 ...
 */
protoRick.disparar(gun, "Fart", "Coach");
console.assert(universo.hasOwnProperty("Coach"));
